...
src/data/       read snapshots, filter, group film+venue+day
src/app/        three routes
src/components/ week grid, day/week chrome, share
```
...
**Day view is still the same page.** `?day=YYYY-MM-DD` is enough. No extra route, no month view.
